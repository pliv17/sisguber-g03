...
use App\Modules\Maestros\Services\BudgetItemService;
use App\Modules\Maestros\Support\MaestroPdf;
use App\Modules\Maestros\Support\Pagination;

final class BudgetItemApiController extends BaseMaestroApiController
{
    public function __construct(private readonly BudgetItemService $service = new BudgetItemService())
    {
    }
    ...
    public function store(Request $request): void
    {
        ...
        JsonResponse::created($this->service->find($id, $year, $code) ?? ['id' => $id, 'year' => $year, 'code' => $code, 'name' => $body['name']]);
    }
    ...
    public function report(Request $request): void
    {
        $this->boot($request);
        $year = $this->yearOrCurrent($request);
        MaestroPdf::stream('partidas-presupuestales.pdf', 'Partidas presupuestales ' . $year, ['Año', 'Código', 'Nombre'], $this->service->rowsForPdf($year, $this->searchQuery($request)));
    }
}
