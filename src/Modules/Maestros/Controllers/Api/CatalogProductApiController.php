...
use App\Modules\Maestros\Services\CatalogProductService;
use App\Modules\Maestros\Support\MaestroPdf;
use App\Modules\Maestros\Support\Pagination;

final class CatalogProductApiController extends BaseMaestroApiController
{
    public function __construct(private readonly CatalogProductService $service = new CatalogProductService())
    {
    }
    ...
    {
        $this->boot($request);
        MaestroPdf::stream('catalogo-bienes.pdf', 'Catálogo de bienes', ['Código', 'Nombre', 'Und. medida'], $this->service->rowsForPdf($this->searchQuery($request)));
    }
}
