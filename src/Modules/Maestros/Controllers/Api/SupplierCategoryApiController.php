...
use App\Modules\Maestros\Services\SupplierCategoryService;
use App\Modules\Maestros\Support\MaestroPdf;
use App\Modules\Maestros\Support\Pagination;

final class SupplierCategoryApiController extends BaseMaestroApiController
{
    public function __construct(private readonly SupplierCategoryService $service = new SupplierCategoryService())
    {
    }
    ...
    {
        $this->boot($request);
        MaestroPdf::stream('rubros-proveedor.pdf', 'Rubros de proveedor', ['Código', 'Descripción'], $this->service->rowsForPdf($this->searchQuery($request)));
    }
}
