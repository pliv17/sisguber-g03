...
use App\Modules\Maestros\Services\OfficeService;
use App\Modules\Maestros\Support\MaestroPdf;
use App\Modules\Maestros\Support\Pagination;

final class OfficeApiController extends BaseMaestroApiController
{
    public function __construct(private readonly OfficeService $service = new OfficeService())
    {
    }
    ...
    {
        $this->boot($request);
        MaestroPdf::stream('oficinas.pdf', 'Oficinas', ['Código', 'Nombre', 'Responsable'], $this->service->rowsForPdf($this->searchQuery($request)));
    }
}
